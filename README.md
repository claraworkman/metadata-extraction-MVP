...
- Supports `.txt`, `.docx`, or `.pdf` formats (with OCR)
- Extracts **11 Sirion metadata fields** using **gpt-4o**
- **Concurrent processing**: 3-5x faster with 10 parallel workers
- **Managed Identity**: Secure authentication for PDF access
- **Markdown extraction**: Automatically saves OCR text to separate blob container
- **Folder structure preservation**: Maintains hierarchy in CSV and markdown output
- Outputs structured CSV ready for Sirion.ai CLM import
...
# Markdown extraction (optional)
MARKDOWN_CONTAINER=extracted-markdown  # Container for OCR text files
SAVE_MARKDOWN=true                     # Save extracted text as markdown
...
**Markdown Extraction**:
- `SAVE_MARKDOWN=true`: Saves OCR-extracted text to blob storage
- `MARKDOWN_CONTAINER=extracted-markdown`: Target container for markdown files
- Preserves folder structure and adds metadata headers
- Useful for full-text search and archival
...
- **Folder Path**: Directory structure within container
- **File Name**: Original filename
- Source language detected
- **11 Sirion metadata fields** (in English)
- Confidence score (high/medium/low)
- Extraction notes

Additionally, OCR-extracted text is saved as markdown files in the `extracted-markdown` blob container.

### Sirion Fields Extracted:
1. **Original File Name** - Complete filename with extension
2. **Counterparty Legal Entity Name** - Supplier/vendor company name
3. **Internal Contracting Entity** - Circle K entity name
4. **Contract Type** - One of 8 document types:
   - Master Agreement
   - Product/ Service Specific Agreement
   ...
   - Order Form
   - Supporting Document
5. **Term Type** - Perpetual, Fixed, or Auto-Renewal
6. **Effective Date** - MM/DD/YYYY format
7. **Expiration Date** - MM/DD/YYYY format (null for Perpetual)
8. **Governing Law** - Jurisdiction (e.g., "Sweden", "Poland")
9. **Payment Term** - Days format (e.g., "Net 30", "Net 45")
10. **Contract Name** - Format: {Type}_{Supplier}_{Date}
11. **Scope Category level 1** - Technology, Real Estate & Construction, or Operations and Logistics

## 💰 Cost

- **~$0.0014 per contract** (using GPT-4o)
